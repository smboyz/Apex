    ...
    return (
        <section className='py-10'>
            <div className="container flex flex-col items-center" data-aos="fade-up" data-aos-duration="1000" data-aos-once="true">
                <h2 className="lg:text-4xl sm:text-3xl text-2xl font-semibold mb-8 relative after:absolute after:w-[80%] after:content-[''] after:h-[4px] after:bg-sky-700 after:bottom-[-8px] after:left-[10%] after:rounded-md before:absolute before:w-[4px] before:h-[4px] before:bg-white before:content-[''] before:z-10 before:bottom-[-8px] custom-animation">Testimonials</h2>
                        ...
                        {data.map((dataItem) => (
                            <div key={dataItem.id} className='flex-col items-center custom-flex bg-gray-100'>
                                <div className='flex items-center gap-4 py-3 px-8 border-b w-full'>
                                    <img className='w-[80px] h-[80px]' src={dataItem.imageUrl} alt={dataItem.name} />
                                    <span className='flex flex-col'>
                                        <h3 className='sm:text-xl text-lg font-bold'>{dataItem.name}</h3>
                                        <i className='my-1 sm:text-base text-xs'>{dataItem.profession}</i>
                                    </span>
                                </div>
                                <span className='inline-block px-5 py-3'>
                                    <p className='text-start sm:text-base text-sm text-gray-700'>{dataItem.message}</p>
                                </span>
                            </div>
                        ))}
                    </Slider>
